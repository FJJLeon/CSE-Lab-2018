...
inode_manager::alloc_inode(uint32_t type)
{
   ...
   */
	...
	if (debug)
		fprintf(stderr, "\tdebug: im::alloc_inode type: %d, get inum %d\n", type, inum);
	return inum;
...
inode_manager::append_block(uint32_t inum, blockid_t &bid)
{
   ...
   * allocate and append a block to the inode and return its id
   */
	debugout("inode_manager::append_block\n");
 
	if (inum <= 0 || inum > INODE_NUM) {
		printf("\terror: inode_manager::append_block inum %d out of range \n",inum);
		return;
	}
	struct inode *ino = get_inode(inum);
	if (ino == NULL)
		printf("\terror: append_block, get inode %d failed\n", inum);

	// get old num of data blocks
	int oldNum = (ino->size + BLOCK_SIZE - 1) / BLOCK_SIZE;
	// alloc new block
	bid = bm->alloc_block();

	blockid_t indirectb[NINDIRECT];
	if (oldNum < NDIRECT) {// direct exists space
		ino->blocks[oldNum] = bid;
	}
	else if (oldNum == NDIRECT) { // oldNum exactly consume all direct
		ino->blocks[NDIRECT] = bm->alloc_block();
		indirectb[0] = bid;
		bm->write_block(ino->blocks[NDIRECT], (char *)indirectb);
	}
	else { // indirect exists
		bm->read_block(ino->blocks[NDIRECT], (char *)indirectb);
		indirectb[oldNum - NDIRECT] = bid;
		bm->write_block(ino->blocks[NDIRECT], (char *)indirectb);
	}
	ino->size += BLOCK_SIZE;
...
inode_manager::get_block_ids(uint32_t inum, std::list<blockid_t> &block_ids)
{
   ...
   * 
   * Given an inode number,
   * return id of all data blocks of the file
   */
    debugout("inode_manager::get_block_ids\n");
	
	if (inum <= 0 || inum > INODE_NUM) {
		printf("\terror: inode_manager::get_block_ids inum %d out of range \n",inum);
		return;
	}
	struct inode *ino = get_inode(inum);
	if (ino == NULL)
		printf("\terror: get_block_ids, get inode %d failed\n", inum);
	...
	// get indirect block id if exists
	blockid_t indirectb[NINDIRECT];
	if (oldNum > NDIRECT) {
		bm->read_block(ino->blocks[NDIRECT], (char *)indirectb);
		for (int i=NDIRECT; i<oldNum; i++){
			block_ids.push_back(indirectb[i - NDIRECT]);
		}
	}
...
inode_manager::read_block(blockid_t id, char buf[BLOCK_SIZE])
{
   ...
   * 
   * Given a block id, 
   * return the data of the disk block
   */
    debugout("inode_manager::read_block\n");
...
inode_manager::write_block(blockid_t id, const char buf[BLOCK_SIZE])
{
   ...
   * your code goes here.
   * 
   * Given a block id and data, 
   * write the data to the disk block
   */
    debugout("inode_manager::write_block\n");
...
inode_manager::complete(uint32_t inum, uint32_t size)
{
   ...
   * your code goes here.
   * 
   * Given an inode number and size, 
   * this request indicates that the writes to the file is finished, 
   * so the metedata (file size, modification time) in inode 
   * could be updated safely
   */
    debugout("inode_manager::complete, inum: %d, newsize %d\n", inum, size);

	if (inum <= 0 || inum > INODE_NUM) {
		debugout("\terror: inode_manager::complete inum %d out of range \n",inum);
		return;
	}
	struct inode *ino = get_inode(inum);
	if (ino == NULL)
		debugout("\terror: complete, get inode %d failed\n", inum);
