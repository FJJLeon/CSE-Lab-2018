...
private:
  ...
  // registered Datanodes with its heartbeat, including master
  std::map<DatanodeIDProto, uint32_t> registerDNs;
  // live Datanodes
  std::list<DatanodeIDProto> liveDNs;
  // master datanode heartbeat
  uint32_t master_hb;
  // block should be replicated
  std::vector<blockid_t> replicated_blocks;
  std::map<yfs_client::inum, uint32_t> appendOffset;
