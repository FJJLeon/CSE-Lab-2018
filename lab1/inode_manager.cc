...
inode_manager::write_file(uint32_t inum, const char *buf, int size)
{
   ...
   */
	...
	//param check
	if (inum <= 0 || inum > INODE_NUM) {
		printf("\terror: inode_manager::write_file inum %d out of range\n",inum);
		return;
	}
	if (buf == NULL) {
		printf("\terror: inode_manager::write_file buf NULL\n");
		return;
	}
	
	//if ((int)strlen(buf) != size) {
	//	printf("\terror: inode_manager::write_file size %d not match buf\n", size);
	//	return;
	//}
	...
	int nowNum = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
	...
	if (oldNum > nowNum) {
		if (nowNum > NDIRECT) { // old > now > NDIRECT
			bm->read_block(indirectbid, (char *)indirectb);
			for (int i=nowNum; i<oldNum; i++)
				bm->free_block(indirectb[i - NDIRECT]);
		}
		else if (oldNum > NDIRECT) { // old > ND > now
			bm->read_block(indirectbid, (char *)indirectb);
			for (int i=nowNum; i<NDIRECT; i++)
				bm->free_block(ino->blocks[i]);
			for (int i=0; i<nowNum - NDIRECT; i++) 
		...
		if (nowNum < NDIRECT) { // ND > now > old
			for (int i=oldNum; i<nowNum; i++)
				ino->blocks[i] = bm->alloc_block();
		}
		else if (oldNum < NDIRECT) { // now > ND > old
			for (int i=oldNum; i<NDIRECT; i++)
				ino->blocks[i] = bm->alloc_block();
			indirectbid = bm->alloc_block();
			ino->blocks[NDIRECT] = indirectbid;
			for (int i=0; i<nowNum - NDIRECT; i++)
				indirectb[i] = bm->alloc_block();
			bm->write_block(indirectbid, (char *)indirectb);
		}
		else { // now > old > ND
			bm->read_block(indirectbid, (char *)indirectb);
			for (int i=oldNum; i<nowNum; i++)
	...
	for(int i=0; i<MIN(nowNum, NDIRECT); i++){
        memcpy(data, buf+i*BLOCK_SIZE, BLOCK_SIZE);
        bm->write_block(ino->blocks[i], data);
    }

    if(nowNum > NDIRECT){
        bm->read_block(ino->blocks[NDIRECT], tmp);
        for(int i=NDIRECT; i<nowNum; i++){
            memcpy(data, buf+i*BLOCK_SIZE, BLOCK_SIZE);
